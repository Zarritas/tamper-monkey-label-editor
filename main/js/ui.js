...
function isDarkMode() {
    ...
    if (html.classList.contains('gl-dark') || 
...
function createGroupElement(groupName, group) {
    ...
    const exclusiveHint = isExclusive ? '' : '<span class="label-group-multi">(múltiple)</span>';
...
function bindPopupEvents(popup, overlay) {
    ...
    popup.querySelector('#label-groups-cancel').addEventListener('click', () => overlay.remove());
    popup.querySelector('#label-groups-submit').addEventListener('click', () => applyChanges(overlay));
...
async function loadProjectLabels(popup) {
    ...
    try {
        ...
        container.innerHTML = '<span class="config-loading">Error al cargar etiquetas</span>';
    }
}
...
function renderProjectLabels(popup) {
    ...
    if (projectLabels.length === 0) {
        container.innerHTML = '<span class="config-loading">No se encontraron etiquetas</span>';
        return;
    }
    ...
    // Bind click events
    container.querySelectorAll('.config-available-label:not(.used)').forEach(el => {
...
function showGroupSelector(labelName, popup, sourceEl) {
    ...
    selector.querySelector('.config-group-selector-cancel').addEventListener('click', () => {
        selector.remove();
    });
    ...
    groupEl.querySelectorAll('.config-label-chip button').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (configGroups[name]) {
                configGroups[name].labels = configGroups[name].labels.filter(l => l !== label);
            }
            chip.remove();
            renderProjectLabels(popup);
            updateJsonEditor(popup);
...
function bindConfigEvents(popup, overlay) {
    ...
    popup.querySelector('#config-save').addEventListener('click', () => {
        // Si estamos en tab JSON, sincronizar primero
        if (currentConfigTab === 'json') {
            if (!syncJsonToVisual(popup)) {
    ...
    popup.querySelector('#config-add-group').addEventListener('click', () => {
        ...
        const newGroupEl = popup.querySelector(`.config-group-item[data-group-name="${newName}"]`);
        if (newGroupEl) {
    ...
    const resetBtn = popup.querySelector('#config-reset');
    if (resetBtn) {
        resetBtn.addEventListener('click', () => {
            if (confirm('¿Restaurar la configuración por defecto? Se perderán los cambios actuales.')) {
                configGroups = JSON.parse(JSON.stringify(DEFAULT_LABEL_GROUPS));
                renderConfigGroups(popup);
                renderProjectLabels(popup);
                updateJsonEditor(popup);
                popup.querySelector('#config-status').innerHTML = 
                    '<div class="label-groups-success">Configuración restaurada</div>';
            }
        });
    }
    ...
    if (refreshBtn) {
        refreshBtn.addEventListener('click', async () => {
            const container = popup.querySelector('#config-available-labels');
            container.innerHTML = '<span class="config-loading">Cargando etiquetas...</span>';
            clearLabelsCache();
...
function handleConfigSave(popup, overlay) {
    ...
    const emptyGroups = Object.entries(configGroups).filter(([name, group]) => 
        !name.trim() || group.labels.length === 0
    );

    if (emptyGroups.length > 0) {
        statusEl.innerHTML = '<div class="label-groups-error">❌ Hay grupos sin nombre o sin etiquetas</div>';
        return;
    }

    saveLabelGroups(configGroups);
    statusEl.innerHTML = '<div class="label-groups-success">✅ Configuración guardada</div>';
